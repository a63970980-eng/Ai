...
app=FastAPI(title='AI Forex Trading Platform',version='1.1.0',docs_url='/docs')
...
class BacktestRequest(MarketRequest): spread:float=0.0; slippage:float=0.0; fee:float=0.0

@app.get('/')
def dashboard():
    p=FRONTEND/'index.html'; return FileResponse(p) if p.exists() else {'service':'AI Forex Trading Platform','docs':'/docs'}
@app.get('/styles.css')
def styles(): return FileResponse(FRONTEND/'styles.css')
@app.get('/app.js')
def script(): return FileResponse(FRONTEND/'app.js')

@app.get('/health')
@app.get('/api/v1/health')
def health(): return {'status':'ok','service':'ai-forex-trading-platform','environment':settings.environment,'live_trading':settings.live_trading_enabled,'paper_trading':settings.paper_trading_enabled,'uptime_since':started.isoformat()}
...
def positions(): return {'positions':[],'mode':'paper'}
@app.get('/api/v1/trades')
def trades(): return {'trades':[],'mode':'paper'}
...
def backtest(request:BacktestRequest):
    ...
    if not {'open','high','low','close'}.issubset(df.columns): raise HTTPException(422,'OHLC columns required')
    return run_backtest(df,lambda _:None,spread=request.spread,slippage=request.slippage,fee=request.fee).__dict__
@app.get('/api/v1/risk')
