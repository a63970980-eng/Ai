...
from forex_robot.domain.models import OrderRequest, Side
...
def test_backtest_has_no_future_access():
    result=run_backtest(candles(), lambda history: momentum("EURUSD", history), spread=.0001)
    assert result.trades >= 0


def test_paper_execution():
    broker=PaperBroker(); gateway=ExecutionGateway(broker)
    oid=gateway.submit(OrderRequest(symbol="EURUSD", side=Side.BUY, units=100, entry=1.1, stop_loss=1.099, take_profit=1.102))
